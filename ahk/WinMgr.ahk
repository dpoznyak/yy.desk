...
SnapActiveWindow(winPlaceVertical, winPlaceHorizontal, winSizeHeight) {
    ...
    WinGet, isMinMax, MinMax, ahk_id %activeWin%
    if (isMinMax != 0) {
    ...
    margin := 0
    WinGet, processName, ProcessName, ahk_id %activeWin%
    if (processName == "chrome.exe") {
        margin := -7
        OutputDebug Adjusting for %processName% by %margin%
    }
    ...
    OutputDebug,  WinMove,A,,%posX%,%posY%,%width%,%height% 

    WinMove,A,,%posX%,%posY%,%width%,%height%

    WinGetPos, posX,posY,width,height,ahk_id %activeWin%

    
    OutputDebug,  after move: %posX%,%posY%,%width%,%height% 
}
